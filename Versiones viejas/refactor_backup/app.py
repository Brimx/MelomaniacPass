"""
╔══════════════════════════════════════════════════════════════════════╗
║       MelomaniacPass v5.0 — Recovery & Universal Auth             ║
║                                                                      ║
...
import requests  # always needed for Apple Music API calls
...
def _spotify_retry_after(exc: BaseException, default: int = 30) -> int:
    """
    Extrae el valor de `Retry-After` de una SpotifyException 429.

    Spotipy expone las cabeceras HTTP en `exc.headers` (dict) cuando el
    cliente subyacente recibe la respuesta de la API. Si el campo no está
    disponible (versión antigua de spotipy o excepción envuelta), devuelve
    `default` segundos como pausa de seguridad.
    """
...
class SearchResult:
    """
    Resultado de búsqueda universal V5.0 ISRC-Master.
    • track_id      : Spotify track ID encontrado (None = no match)
    • needs_review  : True → fuzzy <40% tras Hunter (requiere revisión manual)
    • low_confidence: True → 70–84%: válido; solo log interno
    • isrc          : Código ISRC extraído de external_ids.isrc (solo Spotify)
    """
    track_id: Optional[str] = None
    needs_review: bool = False
    low_confidence: bool = False
    isrc: Optional[str] = None  # ISO 3901 — extraído de Spotify external_ids.isrc
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# §4  SPOTIFY OFFICIAL AUTH — Authorization Code Flow
#     SpotifyShadowAuthManager y cookies sp_dc eliminados en v5.0
#     Autenticación vía SpotifyOAuth + CacheFileHandler (spotipy oficial)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

# Scopes requeridos por MelomaniacPass v5.0
SPOTIFY_REQUIRED_SCOPES = (
    "playlist-modify-public playlist-modify-private user-library-read"
)
# Ruta del cache de tokens OAuth (CacheFileHandler)
...
class MusicApiService:
    ...
    v5.0: Hunter Recovery · semáforo real · post-mortem · cierre seguro de sesiones HTTP
    """
    ...
    def __init__(self, circuit_breakers: dict[str, "CircuitBreaker"]):
        ...
        self.auth_manager = None  # inyectado por main() tras la construcción
        # Apple Music: sesión HTTP compartida (Keep-Alive)
        # User-Agent genérico (no se usa para Spotify en v5.0 — OAuth oficial)
        self._http_session = requests.Session()
        # User-Agent para la sesión HTTP de Apple Music.
        # SPOTIFY_USER_AGENT ya no se usa en v5.0 (SpotifyOAuth gestiona su propia sesión).
        _am_ua = (
    ...
    # ── Public accessors (evitan acceso directo a miembros protegidos) ────

    @property
    def search_cache(self) -> dict:
        """Caché de resultados de búsqueda — acceso público por referencia."""
        return self._search_cache

    def cleanup_sessions(self) -> None:
        """Wrapper público para cerrar sesiones HTTP al apagar la app."""
    ...
    def _sync_init_spotify(self) -> bool:
        """
        Inicialización SILENCIOSA de Spotipy — PROHIBIDAS las llamadas de red.

        Fase 1 ISRC-Master (v5.0):
        • Construye el gestor OAuth con los parámetros del .env.
        • Verifica ÚNICAMENTE la existencia de un token en el caché de disco
          (CacheFileHandler.get_cached_token — solo lectura, sin red).
        • Si no hay token cacheado → retorna False ("Desconectado") de forma
          inmediata para que Flet renderice la UI sin esperas.
        • Si hay token → crea el cliente Spotipy; el primer refresco real se
          hará de forma lazy cuando se realice la primera llamada a la API.
        • NUNCA llama a current_user() ni a ningún endpoint de red aquí.

        Variables de entorno requeridas (.env):
            SPOTIFY_CLIENT_ID      — Client ID de la app en Developer Dashboard
            SPOTIFY_CLIENT_SECRET  — Client Secret de la app
            SPOTIFY_REDIRECT_URI   — URI configurado en el Dashboard
        ...
        try:
            ...
            if not client_id or not client_secret:
                print(
                    "[Spotify] SPOTIFY_CLIENT_ID o SPOTIFY_CLIENT_SECRET "
                    "no configurados en .env"
                )
                return False

            # CacheFileHandler: sólo E/S de disco, cero peticiones de red.
            ...
            oauth = SpotifyOAuth(
                ...
                open_browser=False,  # La app gestiona el navegador desde la UI
            )

            # ── Cortocircuito silencioso: solo verificamos el caché ───────
            # get_cached_token() lee el archivo .spotify_cache sin tocar la red.
            # SpotifyOAuth.get_access_token() refresca automáticamente cuando
            # el cliente hace su primera llamada real a la API.
            token_info = cache_handler.get_cached_token()
            if not token_info:
                # Sin token → "Desconectado". Guardamos el oauth para poder
                # generar la URL de autorización desde el botón "Conectar".
                self._sp_oauth = oauth
                self._sp       = None
                print("[Spotify] Sin token en caché — estado: Desconectado")
                return False

            # Token disponible: cliente lazy (el refresh lo hará spotipy solo)
    ...
    def _safe_sp_call(self, method_name: str, *args, **kwargs):
        """
        Wrapper de llamada segura a la API de Spotify.

        Con SpotifyOAuth, el token se refresca automáticamente al expirar
        (el oauth_manager interno de spotipy lo gestiona).
        En caso de 401 persistente (token revocado / scopes insuficientes),
        se reintenta una vez forzando la lectura de caché + refresh.
        En caso de 429 se lanza RateLimitError con el valor de Retry-After
        para que el CircuitBreaker y el backoff exponencial lo gestionen.
        """
        try:
            ...
            if HAS_SPOTIFY and SpotifyException is not None and isinstance(exc, SpotifyException):
                ...
                if hs == 401:
                    # Un re-init lee el caché actualizado; el refresh lo gestiona spotipy
                    if self._sync_init_spotify() and self._sp:
                        return getattr(self._sp, method_name)(*args, **kwargs)
            raise

    # ── Spotify OAuth — URL & Callback ────────────────────────────────

    def get_spotify_auth_url(self) -> Optional[str]:
        """
        Genera la URL de autorización de Spotify para el flujo manual.

        Llama a `_sync_init_spotify` si el oauth manager aún no está
        construido (solo lectura de caché, sin red). Devuelve la URL
        que el usuario debe abrir en el navegador para autorizar la app.
        Retorna None si no se puede construir la URL (credenciales ausentes).
        """
        if not HAS_SPOTIFY:
            return None
        # Asegurar que el oauth manager existe (solo config, sin red)
    ...
    async def handle_spotify_redirect(self, redirect_response: str) -> bool:
        """
        Completa el intercambio de tokens OAuth con la URL de redirección.

        El usuario pega la URL completa del redirect (o el código) después
        de autorizar la app en el navegador. Este método extrae el código,
        solicita el access_token + refresh_token y persiste ambos en caché.
        Actualiza `self._sp` con el cliente autenticado si tiene éxito.

        Args:
            redirect_response: URL completa (http://127.0.0.1:8888/callback?code=…)
                               o solo el código de autorización.
        Returns:
            True si el intercambio fue exitoso y el cliente está listo.
        """
        if not HAS_SPOTIFY:
            return False
        oauth = getattr(self, "_sp_oauth", None)
        ...
        try:
            code = await asyncio.to_thread(
                oauth.parse_response_code, redirect_response
            )
            if not code or code == redirect_response:
                # parse_response_code devuelve el input original si no encuentra el código
                print("[Spotify] No se pudo extraer el código de la respuesta")
                return False
            ...
            if token_info:
                self._sp = spotipy.Spotify(auth_manager=oauth)
                print("[Spotify] ✓ Autorización completada — cliente listo")
                return True
            return False
        except Exception as exc:  # pylint: disable=broad-exception-caught
    ...
    async def _spotify_search_track_async(
        ...
        """
        Delegador asíncrono hacia _sp_hunter_async.

        En caso de 429 de Spotify extrae el valor exacto de Retry-After
        y eleva RateLimitError para que _search_with_exponential_rl_backoff
        aplique el backoff correcto y el CircuitBreaker notifique la UI.
        PROHIBIDO absorber el 429 con sleep fijo; el tiempo de pausa lo dicta
        la API, no un valor hardcodeado.
        """
        try:
            ...
            raise  # ya viene formateado desde _safe_sp_call / _sp_hunter_async
        except Exception as e:  # pylint: disable=broad-exception-caught
            if _is_spotify_rate_limited(e):
                wait = _spotify_retry_after(e)
                self._cb["Spotify"].trip(wait)
    ...
    async def search_with_fallback(
        ...
        """
        Sniper Mode v5.0 — Búsqueda con Cortocircuito de Alta Confianza.

        ══════════════════════════════════════════════════════════════════
        REGLA CORTOCIRCUITO (Anti-Baneo):
            Si la búsqueda principal (Pase 1) devuelve un resultado de
            alta confianza (track_id ≠ None Y NOT needs_review Y NOT
            low_confidence), la función retorna INMEDIATAMENTE.
            PROHIBIDO iniciar búsquedas de respaldo cuando ya se alcanzó
            el umbral de confianza ≥ 85% en el pase inicial.

        PASES (máx. 3 passes únicas, deduplicadas por clave):
            1. Metadatos depurados: clean_metadata(name, artist)
            2. Original crudo:      name.strip() + artist.strip()
            3. Título normalizado:  _normalize_title(name) + base_artist
            Los pases 4-5 anteriores (experimental/extra noise strip)
            quedan eliminados para reducir el conteo de peticiones API.

        RETORNO TEMPRANO:
            • Pase 1 ≥ 85% (high_confidence)  → retorno inmediato ✓
            • Cualquier pase con track_id found → retorno inmediato ✓
            • Ningún pase encontró nada         → SearchResult(None, False)
        """
        base_t, base_a = clean_metadata(name, artist)

        # ── Pases de búsqueda (3 únicos, sin redundancias) ────────────
        passes: list[tuple[str, str]] = [
            (base_t, base_a),                        # Pase 1: metadatos depurados
            (name.strip(), artist.strip()),           # Pase 2: texto original crudo
            (_normalize_title(name), base_a),        # Pase 3: título normalizado
        ]
        ...
        for idx, (t_pass, a_pass) in enumerate(passes):
            ...
            if result.track_id:
                # ── CORTOCIRCUITO DE ALTA CONFIANZA ───────────────────
                # Pase 1 con alta confianza: retorno inmediato.
                # El cortocircuito interno de _sp_hunter_async ya garantiza
                # que lleguemos aquí solo cuando comb ≥ 85% o artista exacto.
                if idx == 0 and not result.needs_review and not result.low_confidence:
                    return result  # ≥ 85% en el primer pase — sin búsquedas secundarias
                # Cualquier pase con match válido: retorno inmediato
                return result
    ...
    def _sp_search_items(self, q: str) -> list:
        """
        Busca tracks en Spotify usando field operators obligatorios.

        Regla de Query Oficial (Fase 1 ISRC-Master):
            La cadena `q` DEBE llegar pre-formateada con filtros de campo:
                q = 'track:{title} artist:{artist}'
            Esta regla se aplica upstream en `_sp_hunter_async` (structured queries).
            El fallback de texto libre (plain queries) también pasa por este método
            pero sin filtros — se acepta solo como último recurso Hunter.

        Parámetros de API:
            type  = "track"  (obligatorio)
            limit = 10       (límite estricto para dev mode)

        Extracción ISRC:
            Cada ítem devuelto contiene external_ids.isrc cuando está disponible.
            El campo se preserva en el dict para que _build_spotify_result lo capture.

        Returns:
            list[dict] — ítems de tracks (vacío si la API no devuelve resultados).
        """
    ...
    def _build_spotify_result(
        ...
        """
        Construye SearchResult con extracción de ISRC (Fase 1 ISRC-Master).

        El campo external_ids.isrc está presente en todos los tracks de la
        API oficial de Spotify cuando el track tiene código ISRC registrado.
        Se extrae aquí para que el caller pueda usarlo sin re-parsear el dict.
        """
        needs, low = _fuzzy_flags_elastic(comb, tit, art)
    ...
    async def _sp_hunter_async(
        ...
        """
        Búsqueda Spotify con Cortocircuito de Confianza — Sniper Mode v5.0.

        ══════════════════════════════════════════════════════════════════
        FASE 1 — Búsqueda Estructurada (field operators):
            q = 'track:{título} artist:{artista}'
            • Si comb ≥ 85% o _ideal_pass_hunter → CORTOCIRCUITO inmediato.
              PROHIBIDO continuar con la Fase 2 una vez alcanzado el umbral.
            • Hasta 2 queries estructuradas (metadatos depurados + normalizados).

        FASE 2 — Deep Scan (plain text):
            • Solo se activa si best_comb < 60 tras la Fase 1.
            • Si también alcanza ≥ 85% → CORTOCIRCUITO.
            • PROHIBIDO en Fase 2 si Fase 1 ya retornó resultado de alta conf.

        BLINDAJE 429:
            • SpotifyException(429) eleva RateLimitError con Retry-After real.
            • No se absorbe el 429 con sleep fijo; el backoff lo gestiona
              _search_with_exponential_rl_backoff y el CircuitBreaker.
        """
        if not self._sp:
            await asyncio.to_thread(self._sync_init_spotify)
        if not self._sp:
        ...
        # 1. Normalización mínima para el "Cilindrado" de la búsqueda
        nt = _normalize_title(orig_name)
        na = _normalize_title(orig_artist)

        # 2. Priorización de Queries (Calidad sobre Cantidad)
        # Solo 2 estructuradas sin redundancias.
        queries_structured = [
        ...
        # 3. Fase 1: Búsqueda Estructurada (El 95% de los aciertos)
        for q in queries_structured:
            ...
            async with GLOBAL_API_SEMAPHORE:
                try:
                    ...
                    raise  # viene de _safe_sp_call — propagar sin alterar
                except Exception as e:  # pylint: disable=broad-exception-caught
                    if _is_spotify_rate_limited(e):
                        wait = _spotify_retry_after(e)
                        self._cb["Spotify"].trip(wait)
            ...
            if picked:
                # 🔥 CORTOCIRCUITO DE CONFIANZA — Sniper Mode 🔥
                # comb ≥ 85% o artista exacto + título ≥ 60%:
                # resultado encontrado, sin más peticiones.
        ...
        # 4. Fase 2: Deep Scan (Solo si Fase 1 fue mediocre o falló)
        # Si best_comb ≥ 60, la Fase 1 ya tiene un candidato aceptable;
        # no merece la pena gastar otra petición API.
        if best_comb < 60:
        ...
        # 5. Resultado Final (mejor candidato sub-ideal)
        if best_match:
            picked, comb, tit, art = best_match
            return self._build_spotify_result(picked, comb, tit, art)
    ...
    async def _async_sp_create(
        ...
        # ── Regla de los 100 (Fase 1 ISRC-Master) ─────────────────────────────
        # La API de Spotify rechaza con 413 / 429 si se envían más de 100 URIs
        # en un solo request. Fragmentamos en chunks de máximo 100 elementos.
        # Cada chunk usa _one_call para mantener el semáforo + retry ante 429.
        _CHUNK_SIZE = 100
        for _offset in range(0, len(ids), _CHUNK_SIZE):
            _chunk = ids[_offset : _offset + _CHUNK_SIZE]
            await _one_call(sp.playlist_add_items, pl["id"], _chunk)
...
class AppState:
    ...
    async def transfer_playlist(self) -> None:
        ...
        try:
            ...
            for t in selected:
                ...
                # ── Anti-lag yield (Fase 1 ISRC-Master) ──────────────────────────
                # Cede 10 ms al Event Loop de Flet después de cada canción.
                # Imprescindible en transferencias masivas: sin este yield,
                # el hilo de UI se bloquea y la pantalla se congela.
    ...
    def log(self, msg: str) -> None:
        """Wrapper público de _log para acceso desde clases externas."""
        self._log(msg)
...
class PlaylistManagerUI:
    ...
    def __init__(self, page: ft.Page, state: AppState):
        ...
        self.auth_manager             = None  # inyectado por main() tras la construcción
        self._auth_poll_task: Optional[asyncio.Task] = None  # inyectado por main()
    ...
    # ══════════════════════════════════════════════════════════════════
    # SPOTIFY OAUTH — Connect button & callback dialog
    # ══════════════════════════════════════════════════════════════════

    def _on_connect_spotify(self, _e: ft.ControlEvent) -> None:
        """
        Handler del botón 'Conectar Spotify'.

        Flujo (Flet v0.83, open_browser=False):
          1. Solicita la URL de autorización al servicio (sin red).
          2. La lanza con page.launch_url() → navegador por defecto del SO.
          3. Muestra un AlertDialog para que el usuario pegue la URL de
             redirección completa (o solo el código) y finalice el intercambio.
        """
        svc = self.state.service
        auth_url = svc.get_spotify_auth_url()
        if not auth_url:
            self._snack(
                "No se puede generar la URL de Spotify. "
                "Verifica SPOTIFY_CLIENT_ID y SPOTIFY_CLIENT_SECRET en .env",
                error=True,
            )
            return
        # Abre el navegador del SO con la URL de autorización (Flet v0.83)
        self.page.launch_url(auth_url)
        # Muestra el diálogo de recepción del código
        self._open_spotify_oauth_dialog()

    def _open_spotify_oauth_dialog(self) -> None:
        """
        AlertDialog para recibir la URL/código de redirección de Spotify.

        El usuario copia la URL completa (http://127.0.0.1:8888/callback?code=…)
        desde la barra de su navegador y la pega aquí. La app extrae el código
        y finaliza el intercambio de tokens en un hilo de fondo (asyncio.to_thread)
        para no bloquear el hilo principal de Flet.
        """
        ...
        async def _do_exchange(_e: ft.ControlEvent) -> None:
            ...
            if ok:
                ...
                _status_text.value = (
                    ...
                    "Comprueba que la URL es correcta y vuelve a intentarlo."
                )
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Container(
                ...
                content=ft.Column(
                    controls=[
                        ft.Text(
                            "El navegador se ha abierto con la página de Spotify.\n"
                            "Autoriza la aplicación y copia la URL completa "
                            "de la barra de dirección aquí abajo:",
                            size=12,
                            color=TEXT_MUTED,
                            font_family="IBM Plex Sans",
                        ),
    ...
    def _sync_spotify_connect_ui(self, connected: bool) -> None:
        """
        Actualiza el widget de estado Spotify en la barra lateral de forma
        asíncrona y sin bloquear. Llamar desde cualquier corrutina o callback.

        connected=True  → píldora verde "Conectado", botón oculto.
        connected=False → píldora gris  "Desconectado", botón visible.
        """
        ...
        try:
            ...
            pass  # El control puede estar desmontado si se navega durante el update
    ...
    def _build_sidebar(self) -> None:
        ...
        logo = ft.Column([
            ft.Row([
                ...
                ft.Column([
                    ...
                    opacity=1.0),
                    ft.Text("v5.0", size=9, color=TEXT_DIM,
                            font_family="IBM Plex Sans",
        ...
        # ── Spotify — sección de conexión OAuth ───────────────────────────
        # Visible solo cuando el origen o el destino es Spotify.
        # Estado inicial: se evalúa si hay token en caché (sin red).
        _sp_connected_init = (
            HAS_SPOTIFY and bool(s.service._sp)
        )
        self._sp_status_dot = ft.Icon(
            ft.Icons.CIRCLE,
            size=8,
        ...
        self._sp_status_label = ft.Text(
            ...
            size=10,
            color=SUCCESS if _sp_connected_init else TEXT_MUTED,
            font_family="IBM Plex Sans",
        )
        ...
        self._sp_connect_row = ft.Container(
            ...
            border=ft.Border.all(
                0.8,
                SUCCESS if _sp_connected_init else BORDER_LIGHT,
            ),
            border_radius=8,
            padding=ft.Padding.symmetric(horizontal=10, vertical=6),
            visible=(
                s.source == "Spotify" or s.destination == "Spotify"
            ),
        )
        ...
        fixed_top = ft.Column(
            controls=[
                ...
                platform_section,
                self._sp_connect_row,          # ← OAuth Spotify (adaptativo)
                ft.Divider(height=1, color=BORDER_MUTED, thickness=0.5),
    ...
    def _on_state_changed(self) -> None:
        ...
        # ── Spotify connect row — visibilidad y estado adaptativo ──────
        # La fila se muestra solo cuando Spotify está en juego (origen o destino).
        ...
        if sp_in_use:
            ...
            # Borde de la fila: verde si conectado, neutral si no
    ...
    def stop(self) -> None:
        """Cancelación limpia de tareas en curso. Llamar desde hard_cleanup al cerrar."""
        self._stop_skeleton_pulse()
        if self._search_task and not self._search_task.done():
            self._search_task.cancel()

    def start_auth_poll(self, task: asyncio.Task) -> None:
        """Registra la tarea de polling de autenticación (llamar desde main())."""
