...
class MusicApiService:
    ...
    def _cleanup_sessions(self) -> None:
        """Cierra sesiones HTTP abiertas de forma ordenada."""
        # spotipy usa requests.Session internamente
        try:
...
class AppState:
    ...
    async def transfer_playlist(self) -> None:
        """
        Motor de transferencia v2.1
        ────────────────────────────
        • Paralelismo controlado: asyncio.Semaphore(8) + asyncio.gather.
        • Retries con backoff exponencial (3 intentos, 1 s / 2 s / 4 s).
        • Normalización de títulos vía regex antes de buscar.
        • Caché en memoria: evita buscar la misma canción dos veces.
        • Telemetría granular: progreso x/total y consola solo con eventos críticos.
        • Cierre ordenado de sesiones de red al finalizar.
        """

        # 1. Regex y Constantes (Intocables)
        _CLEAN_RE = re.compile(r'\s*[\(\[].*?[\)\]]|\s+feat\.?.*|\s+ft\.?.*', re.IGNORECASE)
        _RUIDO_NEGATIVO = {'live', 'vivo', 'concierto', 'concert', 'remix', 'karaoke', '8d', 'slowed', 'reverb', 'cover'}

        def _normalize(name: str) -> str:
            name = unicodedata.normalize('NFC', str(name))
            cleaned = _CLEAN_RE.sub("", name)
            cleaned = re.sub(r'\s*[-–]\s*$', '', cleaned)
            return " ".join(cleaned.split()).strip().lower()

        def es_asiatico(texto: str) -> bool:
            return bool(re.search(r'[\u4e00-\u9fff\u3040-\u30ff\u3130-\u318f]', texto))

        def validar_match(local_title: str, local_artist: str, remote_result: dict) -> bool:
            # 1. Extracción limpia de la API
            r_title = remote_result.get('title', '').lower()
            # Obtenemos todos los artistas del resultado
            r_artists = [a['name'].lower() for a in remote_result.get('artists', [])]
            r_artist_str = " ".join(r_artists)
            
            # 2. Normalización de lo que tú tienes
            l_title_clean = _normalize(local_title).lower()
            l_artist_clean = _normalize(local_artist).lower()

            # CAPA 0: BYPASS ASIÁTICO (Igual que antes)
            if es_asiatico(l_title_clean) or es_asiatico(r_title):
                return True 

            # CAPA 1: LA PRUEBA DE ÁCIDO (Inclusión Cruzada)
            # Si el título que buscas está en el título de YT O el artista está ahí.
            # Esto salva a Clint Eastwood porque "clint eastwood" in "clint eastwood (remastered)" es True.
            if l_title_clean in r_title or r_title in l_title_clean:
                # Pero ojo, también debe coincidir el artista para no bajar una versión de otro
                if l_artist_clean in r_artist_str or r_artist_str in l_artist_clean:
                    return True

            # CAPA 2: FILTRO DE EXCLUSIÓN (SUAVIZADO)
            # Solo bloqueamos si es un cover o karaoke y tú no pediste eso.
            letales = {'cover', 'karaoke', 'tribute', 'parody'}
            # Solo comparamos contra el título de YouTube
            ruido_remoto = {w for w in letales if w in r_title}
            if ruido_remoto:
                return False

            # CAPA 3: FUZZY MATCHING (Si todo lo anterior falló)
            l_full = f"{l_title_clean} {l_artist_clean}"
            r_full = f"{r_title} {r_artist_str}"
            
            similitud = SequenceMatcher(None, l_full, r_full).ratio()
            return similitud >= 0.65 # Bajamos un poco más para ser tolerantes

        def preparar_query_youtube(titulo: str, artista: str) -> str:
            # Solo quitamos caracteres raros de URL, no la esencia del texto
        ...
        self._log(
            ...
            f"{len(selected)} canciones → {self.destination}"
        )
        self.notify()

        # ── Semáforo: máx. 8 conexiones simultáneas ───────────────────────────
        sem = asyncio.Semaphore(8)
        dest_ids: list[str] = []
        completed_count      = 0

        # ── Coroutine por pista ───────────────────────────────────────────────
        async def _transfer_one(track) -> Optional[str]:
            nonlocal completed_count
            clean_name = _normalize(track.name)
            cache_key  = f"{clean_name.lower()}|||{track.artist.lower()}|||{self.destination}"

            # Caché hit
            if cache_key in self.service._search_cache:
                cached = self.service._search_cache[cache_key]
                if cached:
                ...
                # Notificamos a la UI inmediatamente para ver el progreso en tiempo real
                self.notify()
                return cached
            ...
            async with sem:
                ...
                # Backoff exponencial: 3 intentos (0 s, 1 s, 2 s de espera)
                for attempt in range(3):
                    try:
                        match_id = await self.service.search_track(
                            self.destination, clean_name, track.artist
                        )
                        break  # éxito
                    except RateLimitError:
                        raise   # Rate-limit no se reintenta aquí
                    except Exception as exc:
                        last_exc = exc
                        wait_s   = 2 ** (attempt)   # 1 → 2 → 4 s
                        if attempt < 2:
                            self._log(
                ...
                # Persistir en caché (None si no se encontró)
                self.service._search_cache[cache_key] = match_id
        ...
        try:
            ...
            if dest_ids:
                ...
                if ok:
                    self._log(
                        ...
                        f"{len(dest_ids)}/{len(selected)} · "
                        f"Fallidas: {len(self.failed_tracks)}"
                    )
            ...
            # Cierre ordenado de conexiones de red
            await asyncio.to_thread(self.service._cleanup_sessions)
            self.notify()
...
# ── Design tokens (Notación #AARRGGBB) ────────────────────────────────
BG_DEEP      = "#FF080B14"   # Opaco (FF)
BG_PANEL     = "#FF0D1117"   # Opaco
BG_SURFACE   = "#FF111827"   # Opaco
BG_HOVER     = "#FF1C2434"   # Opaco

# Glassmorphism (Alpha al principio)
# 14 hex ≈ 8% opacidad | 21 hex ≈ 13% opacidad
GLASS_BG     = "#14FFFFFF"   # Blanco con 8% alpha
GLASS_BORDER = "#21FFFFFF"   # Blanco con 13% alpha

ACCENT       = "#FF4F8BFF"   # Azul primario opaco
ACCENT_DIM   = "#FF2D5FCC"   
SUCCESS      = "#FF00D084"   
WARNING      = "#FE2929FF"   
ERROR_COL    = "#FFAD15FF"   

TEXT_PRIMARY = "#FFF0F4FF"
TEXT_MUTED   = "#FF6B7280"
TEXT_DIM     = "#FF374151"

SKELETON_DARK  = "#FF161B22"
SKELETON_LIGHT = "#FF21262D"
...
class SkeletonRow(ft.Container):
    ...
    def __init__(self, index: int):
        ...
        self._thumb  = ft.Container(width=55, height=55, border_radius=6, bgcolor=SKELETON_DARK)
...
def _status_icon(status: str) -> ft.Control:
    icons = {
        "found":       (ft.Icons.CHECK_CIRCLE,    SUCCESS),
        "not_found":   (ft.Icons.CANCEL,           ERROR_COL),
        "searching":   (ft.Icons.LOOP,             ACCENT),
        "transferred": (ft.Icons.CLOUD_DONE,       SUCCESS),
...
class SongRow(ft.Container):
    ...
    def __init__(
        ...
        # ── Thumbnail con ft.Image real ───────────────────────────────────
        self._thumb = ft.Container(
            width=55, height=55,
            border_radius=6,
            bgcolor=SKELETON_DARK,
            clip_behavior=ft.ClipBehavior.ANTI_ALIAS,   # ← requerido por spec
            content=ft.Image(
                src=track.img_url if track.img_url else None,
                fit=ft.BoxFit.COVER,
                error_content=ft.Icon(
                    ft.Icons.MUSIC_NOTE, color=TEXT_DIM, size=18
                ),
            ),
        )
        ...
        self._chk = ft.Checkbox(
            ...
            fill_color={
                ...
                ft.ControlState.DEFAULT:  "#FFFFFF00",
            },
        ...
        title_text = ft.Text(
            ...
            max_lines=1,
            # ← SIN expand=True aquí; el expand va en la Column contenedora
        )
        ...
        row_content = ft.Row(
            controls=[
                ...
                self._thumb,
                # ← Column con tight=True y spacing=0; expand=True en Column
                ft.Column(
    ...
    def refresh(self, track) -> None:
        """Actualiza el estado visual sin reconstruir el widget."""
        self._track = track
        self._chk.value = track.selected
        # Reemplaza ícono de estado (índice 4 en row_content)
    ...
    return ft.Button(
        ...
        style=ft.ButtonStyle(
            bgcolor={ft.ControlState.DEFAULT: ACCENT,
                     ft.ControlState.HOVERED: "#6B9FFF",
...
def _ghost_btn(text: str, icon: str, on_click, width=None) -> ft.OutlinedButton:
    return ft.OutlinedButton(
        ...
        style=ft.ButtonStyle(
            ...
            side={ft.ControlState.DEFAULT: ft.BorderSide(1, "#2D3748"),
...
class PlaylistManagerUI:
    ...
    def __init__(self, page: ft.Page, state: AppState):
        ...
        self.root = ft.Container(
            ...
            bgcolor=BG_DEEP, # Esto pone una capa sólida DEBAJO de la sidebar
            expand=True,
        )
    ...
    def _build_sidebar(self) -> None:
        ...
        logo = ft.Column([
            ft.Row([
                ft.Icon(ft.Icons.HEADPHONES, color=ACCENT, size=26),
        ...
        self._src_dd = ft.Dropdown(
            ...
            on_select=lambda e: s.set_source(e.control.value),
        )
        self._dst_dd = ft.Dropdown(
            ...
            text_style=ft.TextStyle(color=TEXT_PRIMARY, size=13,
                                    font_family="IBM Plex Sans"),
            border_radius=10, expand=True,
            on_select=lambda e: s.set_destination(e.control.value),
        )
        ...
        self._load_btn = _primary_btn(
            "Cargar Playlist", ft.Icons.DOWNLOAD,
            self._on_load, width=None,
        )
        self._transfer_btn = _ghost_btn(
            "Transferir", ft.Icons.SWAP_HORIZ,
            self._on_transfer,
        )
        self._sync_btn   = _ghost_btn("Sincronizar", ft.Icons.SYNC,
                                      lambda _: self._snack("Función próximamente"))
        self._split_btn  = _ghost_btn("Dividir",     ft.Icons.CALL_SPLIT,
                                      lambda _: self._snack("Función próximamente"))
        self._delete_btn = _ghost_btn("Eliminar",    ft.Icons.DELETE_OUTLINE,
                                      lambda _: self._snack("Función próximamente"))
        ...
        self._rl_banner = ft.Container(
            content=ft.Row([
                ft.Icon(ft.Icons.TIMER, color=WARNING, size=16),
    ...
    def _build_content(self) -> None:
        ...
        # ── Column headers ────────────────────────────────────────────
        def _col_header(text: str, width=None, expand=False, text_align=ft.TextAlign.CENTER) -> ft.Container:
            # Usamos la clase Alignment con constantes en MAYÚSCULAS
            # Si el texto es LEFT, el contenedor se alinea a CENTER_LEFT (centrado vertical, izquierda horizontal)
            c_align = ft.Alignment.CENTER if text_align == ft.TextAlign.CENTER else ft.Alignment.CENTER_LEFT
            
            ctrl = ft.Text(
                text, size=10, color=TEXT_DIM, weight=ft.FontWeight.W_700,
                font_family="IBM Plex Sans",
                style=ft.TextStyle(letter_spacing=0.8),
                text_align=text_align, 
            )
            return ft.Container(content=ctrl, width=width, expand=expand, alignment=c_align)

        col_headers = ft.Container(
            content=ft.Row(
                controls=[
                    _col_header("#",               width=32),
                    _col_header("PORTADA",         width=55),
                    # Aplicamos TextAlign.LEFT (Mayúsculas)
                    _col_header("TÍTULO / ARTISTA", expand=True, text_align=ft.TextAlign.LEFT),
        ...
        self._empty_state = ft.Container(
            content=ft.Column(
                controls=[
                    ft.Icon(ft.Icons.LIBRARY_MUSIC, size=64, color=TEXT_DIM),
    ...
    def _on_state_changed(self) -> None:
        ...
        if s.transfer_total:
            ...
            # --- INGENIERÍA: Estado Derivado ---
            fallidas = len(s.failed_tracks)
            exitosas = s.transfer_progress - fallidas
            porcentaje = int(frac * 100)
            
            prog_label = self._progress_row.content.controls[1].controls[0]
            prog_label.value = f"{porcentaje}%  •  {exitosas} éxitos  /  {fallidas} errores"
        ...
        # ── Diálogo de errores post-transferencia ─────────────────────────────
        if s.transfer_state == TransferState.DONE:
            self._show_failed_dialog()
        elif s.transfer_state == TransferState.RUNNING:
            self._failed_dialog_shown = False   # reset para la siguiente transferencia
    ...
    def _show_failed_dialog(self) -> None:
        """
        Muestra un AlertDialog con el resumen de pistas no encontradas.
        Incluye botón 'Exportar Errores' que escribe failed_tracks.log.
        Solo se muestra una vez por transferencia (_failed_dialog_shown flag).
        """

        failed = getattr(self.state, "failed_tracks", [])
        if not failed or self._failed_dialog_shown:
            return
        self._failed_dialog_shown = True

        # ── Construcción de items del ListView ───────────────────────────────
        items = [
            ft.Container(
                content=ft.Row(
                    controls=[
                        ft.Icon(ft.Icons.WARNING_AMBER_ROUNDED,
                                color=ERROR_COL, size=14),
                        ft.Text(
                            f"{t.name[:44]} — {t.artist[:28]}",
        ...
        def _export_errors(_) -> None:
            log_path = "failed_tracks.log"
            lines = [
                f"# Playlist Manager — Pistas no encontradas\n",
                f"# Fecha: {__import__('datetime').datetime.now().isoformat()}\n",
                f"# Destino: {self.state.destination}\n\n",
            ]
            for t in failed:
        ...
        dlg = ft.AlertDialog(
            ...
            title=ft.Row(
                controls=[
                    ft.Icon(ft.Icons.ERROR_OUTLINE, color=ERROR_COL, size=20),
                    ft.Text(
                        f"Pistas no encontradas · {len(failed)} / {self.state.transfer_total}",
            ...
            content=ft.Container(
                content=failed_lv,
                width=520,
                height=min(320, len(failed) * 36 + 16),
                bgcolor=BG_SURFACE,
                border_radius=8,
                padding=ft.Padding.all(4),
            ),
...
async def main(page: ft.Page) -> None:
    ...
    def _exception_handler(loop, context: dict) -> None:
        ...
        # Suprimir ConnectionResetError (WinError 10054) de pipes de asyncio
        if isinstance(exc, (ConnectionResetError, BrokenPipeError)):
            return
        # Suprimir AttributeError en _call_connection_lost (pipe ya cerrado)
        msg = context.get("message", "")
        if "_call_connection_lost" in msg or "shutdown" in msg:
            return
        # Todo lo demás va al handler por defecto
