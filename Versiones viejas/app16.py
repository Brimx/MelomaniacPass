...
class MusicApiService:
    ...
    def _sync_init_spotify(self) -> bool:
        """
        Inicializa Spotipy mediante el flujo oficial Authorization Code Flow.

        Fase 1 ISRC-Master:
        • Usa SpotifyOAuth (spotipy.oauth2) en lugar del shadow auth (sp_dc).
        • El token se persiste en disco via CacheFileHandler (.spotify_cache).
        • open_browser=False: el usuario pega el redirect_uri manualmente en
          el wizard o via CLI cuando no hay token cacheado todavía.
        • SpotifyOAuth refresca el access_token automáticamente usando el
          refresh_token almacenado en cache → no requiere intervención manual.

        Variables de entorno requeridas (.env):
            SPOTIFY_CLIENT_ID      — Client ID de la app en Developer Dashboard
            SPOTIFY_CLIENT_SECRET  — Client Secret de la app
            SPOTIFY_REDIRECT_URI   — URI configurado en el Dashboard (default: http://127.0.0.1:8888/callback)
        """
        if not HAS_SPOTIFY:
            return False
        try:
            ...
            # CacheFileHandler persiste el token entre reinicios de la app
            ...
            oauth = SpotifyOAuth(
                ...
                open_browser=False,  # La app no lanza el navegador por sí sola
            )
            self._sp = spotipy.Spotify(auth_manager=oauth)
            self._sp.current_user()   # Valida que el token es válido
            return True
        except Exception as exc:  # pylint: disable=broad-exception-caught
            # spotipy.oauth2 puede lanzar SpotifyOauthError u otras excepciones
    ...
    def _safe_sp_call(self, method_name: str, *args, **kwargs):
        ...
        se reintenta una vez con re-init forzado para releer el cache.
        """
        try:
            ...
            if (
                ...
                # Intenta un re-init (releerá el cache / solicitará refresh_token)
                if self._sync_init_spotify():
                    return getattr(self._sp, method_name)(*args, **kwargs)
            raise
    ...
    async def _sp_hunter_async(
        ...
        """
        Optimización v5.0: Búsqueda de alta velocidad con Cortocircuito de Confianza.
        Reduce drásticamente las llamadas a la API para evitar Error 429.
        """
        if not self._sp:
            await asyncio.to_thread(self._sync_init_spotify)
        if not self._sp:
        ...
        # Eliminamos redundancias: solo 2 estructuradas y 1 plana de emergencia.
        queries_structured = [
        ...
        for q in queries_structured:
            ...
            if picked:
                # 🔥 CORTOCIRCUITO DE CONFIANZA 🔥
                # Si el match es >= 85%, es la canción. No gastamos más peticiones.
                if comb >= 85 or _ideal_pass_hunter(comb, tit, art):
        ...
        # 4. Fase 2: Deep Scan (Solo si la Fase 1 fue mediocre o falló)
        # Si ya tenemos un match decente (>60%), evitamos el Deep Scan para ahorrar API.
        if best_comb < 60:
