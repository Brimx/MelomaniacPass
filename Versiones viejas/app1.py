...
class YouTubeMusicClient:
    ...
    Usa la librería ytmusicapi. Requiere archivo oauth.json.
    """
    def __init__(self):
        self.client = None

    def login(self):
        # ytmusicapi usa un archivo JSON para guardar las credenciales OAuth
        if os.path.exists("oauth.json"):
            self.client = YTMusic("oauth.json")
            return True
        else:
            print("Error: No se encontró oauth.json para YouTube Music.")
            # Si quisieras, aquí podrías invocar YTMusic.setup() pero detendría la UI
            return False
...
class AppleMusicClient:
    ...
    Nota: Apple Music requiere un Developer Token (JWT) generado con una cuenta de pago de Apple Developer ($99/año)
    y un Music User Token. Se implementa usando requests como base para cuando se tengan los tokens.
    """
    def __init__(self):
        self.dev_token = os.getenv("APPLE_DEV_TOKEN")
        self.user_token = os.getenv("APPLE_USER_TOKEN")
        self.base_url = "https://api.music.apple.com/v1"
        self.headers = {}

    def login(self):
        if not self.dev_token or not self.user_token:
            print("Error: Faltan tokens de Apple Music en el .env")
            return False
