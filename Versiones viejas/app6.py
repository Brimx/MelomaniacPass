...
class Auth:
    ...
    def login_youtube(self):
        """
        Autentica con YouTube Music leyendo los headers de browser auth
        directamente desde el archivo .env — sin necesidad de browser.json.

        Variables en .env:
            YTM_COOKIE          → (obligatorio) valor completo de la cabecera Cookie
            YTM_USER_AGENT      → (recomendado) User-Agent del navegador
            YTM_AUTH_USER       → valor de X-Goog-AuthUser, normalmente "0"
            YTM_VISITOR_ID      → valor de X-Goog-VisitorId (copiado de DevTools)

        Cómo obtener los valores (una sola vez):
            1. Abre music.youtube.com en Chrome/Firefox con tu cuenta iniciada.
            2. DevTools (F12) → Network → recarga la página → clic en cualquier
               petición a music.youtube.com → pestaña Headers → Request Headers.
            3. Copia el valor completo de 'Cookie' (la línea más larga).
            4. Copia también 'User-Agent', 'X-Goog-AuthUser' y 'X-Goog-VisitorId'.
            5. Pégalos en .env: YTM_COOKIE="...", YTM_USER_AGENT="...", etc.

        ytmusicapi acepta un JSON string como parámetro 'auth', equivalente al
        contenido de browser.json, lo que permite centralizar todo en el .env.
        """
        import json

        # ── 1. Leer variables del .env ─────────────────────────────────────
        cookie     = os.getenv("YTM_COOKIE", "").strip()
        user_agent = os.getenv("YTM_USER_AGENT", "").strip()
        auth_user  = os.getenv("YTM_AUTH_USER", "0").strip()
        visitor_id = os.getenv("YTM_VISITOR_ID", "").strip()

        # Cookie es el único campo estrictamente obligatorio
        if not cookie:
            print("❌ YTM_COOKIE no encontrado o vacío en el .env.")
            print("   → Agrega YTM_COOKIE='<valor>' en tu archivo .env")
            print("   → Obtén el valor desde DevTools → Network → Request Headers → Cookie")
            return None

        # ── 2. Construir el dict de headers que ytmusicapi espera ──────────
        # El formato es idéntico al JSON que genera 'ytmusicapi browser'.
        # ytmusicapi acepta el dict serializado como JSON string en 'auth'.
        headers = {
            "Cookie":           cookie,
        ...
        if visitor_id:
            # X-Goog-VisitorId es necesario en algunas regiones para que
            # ytmusicapi calcule correctamente la firma SAPISIDHASH interna.
            headers["X-Goog-VisitorId"] = visitor_id

        # ── 3. Cargar sesión y validar con petición de prueba ──────────────
        # YTMusic.auth acepta: ruta a archivo, JSON string, o dict (según versión).
        # Serializar a JSON string garantiza compatibilidad con todas las versiones.
        try:
            client = YTMusic(json.dumps(headers))
            # Petición mínima para confirmar que las cookies siguen activas
            client.get_library_playlists(limit=1)
            print("✓ YouTube Music: sesión cargada correctamente desde .env.")
            return client
            ...
            if any(k in err_low for k in ("cookie", "401", "403", "expired", "unauthorized")):
                print("❌ Sesión expirada o Cookie inválida.")
                print("   → Actualiza YTM_COOKIE en el .env con una Cookie fresca del navegador.")

            elif "oauth_credentials" in err_low:
                # No debería ocurrir con headers dict, pero por seguridad
                print("❌ ytmusicapi recibió un formato OAuth inesperado.")
                print("   → Verifica que YTM_COOKIE no sea un token OAuth sino una Cookie de navegador.")

            else:
                print(f"❌ Error al cargar YouTube Music: {err}")
...
class SpotifyClient:
    ...
    def create_playlist(self, title, song_uris):
        if not self.sp: return False, "Spotify no inicializado"
        try:
...
class YouTubeMusicClient:
    ...
    def get_playlist_songs(self, playlist_id):
        ...
        try:
            ...
            for track in playlist.get('tracks', []):
                # Tracks no disponibles (privados/eliminados) llegan sin videoId
    ...
    def create_playlist(self, title, video_ids):
        if not self.client: return False, "YouTube Music no inicializado"
        try:
...
class AppleMusicClient:
    ...
    def login(self):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                return True
            print(f"Apple Music: login falló con status {resp.status_code}")
            return False
        except Exception as e:
            print(f"Apple Music: error al validar sesión: {e}")
            return False
    ...
    def get_playlist_songs(self, playlist_id):
        ...
        try:
            ...
            print(f"Apple Music: no se pudo obtener nombre de playlist: {e}")
        ...
        try:
            ...
            print(f"Apple Music: error al obtener canciones (parcial): {e}")
            return {"name": playlist_name, "tracks": all_songs}
    ...
    def search_song(self, title, artist):
        ...
        try:
            ...
            print(f"Apple Music: error en búsqueda '{title}': {e}")
            return None

    def create_playlist(self, title, track_ids):
        if not self.headers: return False, "Apple Music no inicializado"
        ...
        def _task():
            ...
            if client.login():
                ...
                if data["tracks"]:
                    ...
                    self.root.after(0, lambda: self._log("✓ Lista cargada. Desliza o usa 'Cargar todo'."))
                else:
                    self.root.after(0, lambda: self._log("✗ No se encontraron canciones."))
