...
# Dependency Inversion Principle: importamos desde la librería oficial, no de un archivo local propio.
# CacheFileHandler serializa/deserializa token_info como JSON en disco y aplica chmod 600 al archivo.
...
class SpotifyShadowAuthManager:
    """
    Arquitectura: Custom Authentication Manager con persistencia de sesión en disco.

    PATRÓN APLICADO — Duck Typing + Dependency Injection:
        Spotipy espera un objeto con el método `get_access_token()`. No importa
        su tipo: si lo tiene, funciona. Le inyectamos un `cache_handler` externo
        para que esta clase no decida CÓMO ni DÓNDE se guarda el token.

    FLUJO DE DATOS (tres capas):
        ┌─────────────────────────────────────────────────────────┐
        │ DISCO (.cache-shadow-spotify)                           │
        │   └─► get_cached_token() ─► ¿token válido?             │
        │                                   │ Sí → retornar      │
        │                                   │ No ↓               │
        │ RED (Shadow API de Spotify)        │                    │
        │   └─► requests.get(sp_dc) ────────┘                    │
        │         └─► save_token_to_cache() ─► DISCO             │
        │               └─► MEMORIA (retornar a Spotipy)         │
        └─────────────────────────────────────────────────────────┘

    De esta forma, la cookie `sp_dc` solo se usa cuando el token expiró
    o no existía en disco, minimizando las llamadas a la Shadow API.
    """

    def __init__(self, cache_handler: CacheFileHandler):
        """
        Parámetros:
            cache_handler: Cualquier instancia que implemente
                           get_cached_token() / save_token_to_cache().
                           Usamos CacheFileHandler para persistencia en disco.
        """
        # Inyección de Dependencias: esta clase no sabe si el handler
        # escribe en disco, en Redis o en memoria. Solo llama su interfaz.
    ...
    def get_access_token(self, as_dict=False):
        ...
        Orden de resolución del token (Cache-Aside Pattern):
          1. Leer caché de disco  → retornar si válido (evita llamada HTTP)
          2. Llamar Shadow API    → si caché vacía o expirada
          3. Persistir en disco   → para que el próximo arranque no llame HTTP
          4. Retornar token       → respetando el flag `as_dict`
        """
        # ── CAPA 0: BYPASS MANUAL (Para pruebas) ──────────────────────────
        # Si pones el Bearer directamente en el .env, lo usamos y saltamos todo.
        manual_bearer = os.getenv("SPOTIFY_MANUAL_BEARER", "").strip()
        if manual_bearer:
            # Quitamos la palabra 'Bearer ' si la pegaste por error
            token = manual_bearer.replace("Bearer ", "")
            # print("⚠️ Spotify: Usando Bearer MANUAL (Expira en 1 hora)")
            return {"access_token": token} if as_dict else token
        
        # ── CAPA 1: DISCO ─────────────────────────────────────────────────
        # Intentamos leer el token_info guardado en el archivo local.
        # token_info tiene la forma: {"access_token": "...", "expires_at": 1234567890}
        token_info = self.cache_handler.get_cached_token()

        if token_info:
            expires_at = token_info.get("expires_at", 0)
            # Margen de seguridad de 5 minutos (300 s) para renovar antes de expirar.
            # Evita que Spotipy use un token que expirará a mitad de una operación.
            if time.time() < (expires_at - 300):
                print("✓ Spotify: Token leído desde caché en disco. Sin llamada HTTP.")
                token = token_info["access_token"]
                return {"access_token": token} if as_dict else token

        # ── CAPA 2: RED (Shadow API) ───────────────────────────────────────
        # Solo llegamos aquí si el token no existía en disco o estaba expirado.
        print("🔄 Spotify: Token ausente o expirado en disco. Renovando vía Shadow API...")
        ...
        # La Shadow API devuelve `accessTokenExpirationTimestampMs` en milisegundos.
        # Lo convertimos a segundos Unix para ser compatibles con CacheFileHandler
        # (mismo formato que usa SpotifyOAuth estándar de Spotipy).
        raw_expiration_ms = data.get("accessTokenExpirationTimestampMs")
        if raw_expiration_ms:
            expires_at = int(raw_expiration_ms / 1000)
        else:
            # Fallback conservador: 55 minutos (tokens de web-player duran ~1h)
            expires_at = int(time.time()) + 3300

        access_token = data.get("accessToken")
        if not access_token:
            raise Exception("La Shadow API no devolvió un accessToken válido.")

        # ── CAPA 3: PERSISTENCIA EN DISCO ─────────────────────────────────
        # Construimos el diccionario con el formato estándar de Spotipy
        # para que CacheFileHandler pueda serializarlo/deserializarlo con JSON.
        token_info = {
            "access_token": access_token,
            "expires_at":   expires_at,
        }

        # Guardamos en disco. La próxima vez que arranque la app,
        # la CAPA 1 lo leerá directamente sin tocar la Shadow API.
        self.cache_handler.save_token_to_cache(token_info)
        print(f"✓ Spotify: Token nuevo guardado en disco (expira: "
              f"{time.strftime('%H:%M:%S', time.localtime(expires_at))}).")

        # ── CAPA 4: RETORNO A SPOTIPY ─────────────────────────────────────
        # Respetamos el contrato del duck typing: Spotipy puede pedir
        # el token como string o como dict según el contexto interno.
        return {"access_token": access_token} if as_dict else access_token
...
class Auth:
    ...
    def login_spotify(self):
        """
        Login de Spotify con persistencia de token en disco.

        PATRÓN APLICADO — Composition Root + Dependency Injection:
            Este método es el único lugar donde se decide QUÉ implementación
            concreta de cache_handler se usa. Si mañana quisieras cambiar a
            RedisCacheHandler, solo cambias estas dos líneas sin tocar
            SpotifyShadowAuthManager ni SpotifyClient.

        FLUJO DE CONSTRUCCIÓN (wiring de dependencias):
            Auth.login_spotify()
              │
              ├─► CacheFileHandler(".cache-shadow-spotify")  [escribe en disco]
              │       └─► inyectado en ▼
              └─► SpotifyShadowAuthManager(cache_handler=...)
                      └─► inyectado en ▼
                  spotipy.Spotify(auth_manager=...)
                      └─► llama get_access_token() en cada request
        """
        try:
            # 1. Instanciar el handler de caché que escribe en disco local.
            #    El archivo '.cache-shadow-spotify' vive junto a app.py.
            #    CacheFileHandler aplica chmod 600 al archivo automáticamente.
            cache_handler = CacheFileHandler(cache_path=".cache-shadow-spotify")

            # 2. Inyectamos el handler en nuestro motor de tokens personalizado.
            #    SpotifyShadowAuthManager ya no gestiona su propio almacenamiento.
            auth_manager = SpotifyShadowAuthManager(cache_handler=cache_handler)

            # 3. Forzamos la primera obtención para validar que el .env está bien
            #    y que la caché/Shadow API responden correctamente antes de continuar.
            auth_manager.get_access_token()

            # 4. Inicializamos Spotipy pasando nuestro auth_manager.
            ...
            # Smoke test: petición ligera para confirmar que el token funciona
            # con la API real de Spotify (no solo que el objeto existe).
            self.sp.current_user()
            print("✓ Spotify: Sesión Shadow iniciada con persistencia en disco.")
            return True
...
class AppleMusicClient:
    ...
    def login(self):
        ...
        if not headers: 
            print("❌ Apple Music: No se encontraron tokens en el .env")
            return False

        try:
            # Intentamos obtener el storefront (país)
            ...
            if resp.status_code == 200:
                ...
                print(f"❌ Apple Music: Error de login. Código: {resp.status_code}")
                print(f"   Respuesta del servidor: {resp.text}")
                return False
        except Exception as e:
            print(f"❌ Apple Music: Error crítico de conexión: {e}")
            return False
