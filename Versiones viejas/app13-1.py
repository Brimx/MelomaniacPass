"""
╔══════════════════════════════════════════════════════════════════════╗
║     Playlist Manager v4.5 — Senior Overhaul · Fully Wired           ║
║                                                                      ║
║  Architecture : BLoC-inspired (AppState ◄─ Service ◄─ UI)           ║
║  Design       : Solid dark surfaces · IBM Plex Sans · OLED          ║
║  Engine       : Hunter Raw · Per-platform Auth · Overlay Dialog      ║
║  Lifecycle    : os._exit · Proactive probe · Semáforo real           ║
║                                                                      ║
║  v4.5 CHANGES                                                        ║
║  • §1  check_all_sessions fires proactively after _startup           ║
║  • §2  Auth-icon clicks open wizard, navigate to failing tab         ║
║  • §3  Progress panel border turns green; never hidden post-done     ║
║  • §4  Post-mortem AlertDialog fires on DONE and on ERROR            ║
║  • §5  raw_text_search integrated as mandatory 3rd-pass fallback     ║
║  • §6  hard_cleanup cancels ALL asyncio tasks; os._exit(0) on close  ║
║  • Fix: ft local variable shadow renamed → f_title in service        ║
║  • Fix: _auth_poll_task initialised in __init__                      ║
║  • Fix: wizard receives failing platform for direct tab navigation   ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
from auth_manager import AuthManager, read_browser_json, ENV_FILE, BROWSER_JSON
...
def _fuzzy_score_pair(
    ...
    ft, fa = clean_metadata(found_title, found_artist)
    return int(
...
def _fuzzy_scores_triple(
    ...
    tit = int(_fuzz.token_sort_ratio(ct.lower(), ft.lower()))
...
class MusicApiService:
    ...
    v4.3: Hunter Recovery · semáforo real · post-mortem · cierre seguro de sesiones HTTP
    """
    ...
    def _sync_init_spotify(self) -> bool:
        ...
        try:
            ...
            sp_sess.headers.update(spotify_navigation_headers())
            self._sp = spotipy.Spotify(auth_manager=mgr, requests_session=sp_sess)
    ...
    async def raw_text_search(
        ...
        """
        FIX4 – Hunter Raw: último intento por texto plano sin operadores.
        Diseñado para artistas como Joji cuyo nombre corto engaña a los motores.
        """
        raw_q = f"{name.strip()} {artist.strip()}".strip()
        if not raw_q:
            return SearchResult(None, False)
        ...
        if platform == "YouTube Music":
            ...
            try:
                async with GLOBAL_API_SEMAPHORE:
                    results = await asyncio.to_thread(
                        self._ytm.search, raw_q, filter="songs", limit=5
                    )
                if results:
                    vid = _yt_select_best(name, artist, results, local_duration_s)
                    if vid:
                        chosen = next((r for r in results if r.get("videoId") == vid), None)
                        if chosen:
                            return self._yt_pack_result(chosen, name, artist)
            except Exception:
                pass
            ...
            try:
                async with GLOBAL_API_SEMAPHORE:
                    items = await asyncio.to_thread(self._sp_search_items, raw_q)
                if items:
                    best_t, comb, tit, art = self._sp_pick_best_item(
                        items, name, artist, local_duration_s
                    )
                    if best_t:
                        return self._build_spotify_result(best_t, comb, tit, art)
            except Exception:
                pass
            ...
            try:
                async with GLOBAL_API_SEMAPHORE:
                    cands = await asyncio.to_thread(self._am_candidates_for_term, raw_q)
                return self._am_pick_catalog_best(name, artist, cands)
            except Exception:
                pass
    ...
    def _yt_pack_result(self, chosen: dict, orig_name: str, orig_artist: str) -> SearchResult:
        # NOTE: renamed from `ft` to `f_title` to avoid shadowing the `flet` module import.
        f_title = chosen.get("title", "")
        farts = ", ".join(
            a.get("name", "")
            for a in (chosen.get("artists") or [])
            if isinstance(a, dict)
        )
        comb, tit, art = _fuzzy_scores_triple(orig_name, orig_artist, f_title, farts)
    ...
    async def _yt_hunter_async(
        ...
        """Hunter + Deep Scan: consultas estructuradas, luego texto plano y Joji."""
        if not self._ytm:
            return SearchResult(None, False)
        ...
        for q in (
            ...
            f"{nt} {na}".strip(),
            nt or ct,
            f"{orig_name.strip()} {orig_artist.strip()}",
    ...
    def _am_pick_catalog_best(
        ...
        if not HAS_RAPIDFUZZ:
            ...
            ft, fa = parts[0], parts[1] if len(parts) > 1 else ""
            comb, tit, art = _fuzzy_scores_triple(song_title, artist_name, ft, fa)
            needs, low = _fuzzy_flags_elastic(comb, tit, art)
    ...
    async def _am_hunter_async(
        ...
        for t in (
            f"{ct} {ca}",
            f"{_normalize_title(orig_name)} {_normalize_title(orig_artist)}".strip(),
    ...
    def _sp_search_items(self, q: str) -> list:
        r = self._sp.search(q=q, type="track", limit=10)
    ...
    async def _async_sp_create(
        ...
        pl = await _one_call(
            ...
            True,
            False,
            "Transferida por Playlist Manager",
        )
...
class AppState:
    ...
    def __init__(self, service: MusicApiService):
        ...
        # v4.5: None = "checking" (dim icon); True = OK (green); False = fail (red)
        self.auth_session_ok: dict[str, Optional[bool]] = {p: None for p in self.PLATFORMS}
        self.auth_session_hint: dict[str, str] = {p: "Validando sesión…" for p in self.PLATFORMS}
    ...
    async def load_playlist(self, playlist_id: str) -> None:
        ...
        # Launch background availability scan (lazy loading)
        if self.load_state == LoadState.READY and self.tracks:
            self._lazy_task = asyncio.create_task(
                self._lazy_availability_scan(list(self.tracks))
            )
    ...
    async def transfer_playlist(self) -> None:
        ...
        async def _transfer_one(track: Track) -> Optional[str]:
            ...
            if match.track_id and match.needs_review:
                ...
                # FIX4D — Hunter Raw: último intento sin operadores (Joji / Fantasy)
                if last_exc is None:
                    try:
                        raw_match = await self.service.raw_text_search(
                            self.destination, track.name, track.artist, local_dur_s
                        )
                    except Exception:
                        raw_match = SearchResult(None, False)
                    if raw_match.track_id and not raw_match.needs_review:
                        match = raw_match
                        track.transfer_status = "found"
                        self.count_processed += 1
                        self.service._search_cache[cache_key] = raw_match
                        self._log(f"[SUCCESS] ✓ Hunter Raw: {track.name[:42]}")
                    elif raw_match.track_id and raw_match.needs_review:
                        match = raw_match
                        track.transfer_status = "revision_necesaria"
                        track.failure_reason = "Hunter Raw: fuzzy <40%"
                        self.service._search_cache[cache_key] = raw_match
                        if track not in self.pending_review_tracks:
                            self.pending_review_tracks.append(track)
                        self._log(f"[WARN]  ⚠ Hunter Raw revisión: {track.name[:42]}")
                    else:
                        track.transfer_status = "not_found"
                        track.failure_reason = "Sin resultados (Hunter + Raw)"
        ...
        try:
            ...
            tasks   = [_transfer_one(t) for t in selected]
...
class SongRow(ft.Container):
    ...
    def __init__(self, track: Track, index: int, on_toggle: Callable[[str], None]):
        ...
        self._thumb = ft.Container(
            ...
            content=ft.Image(
                src=track.img_url if track.img_url else None,
                fit=ft.BoxFit.COVER,
...
def _primary_btn(text: str, icon: str, on_click, width=None) -> ft.Button:
    return ft.Button(
...
class PlaylistManagerUI:
    ...
    def __init__(self, page: ft.Page, state: AppState):
        ...
        self._auth_poll_task:    Optional[asyncio.Task] = None  # v4.5: init here
    ...
    def _on_auth_probe(self, _) -> None:
        asyncio.create_task(self._refresh_auth_live())

    async def _on_platform_auth_click(self, platform: str) -> None:
        """
        FIX1: Valida la sesión de UNA plataforma concreta al hacer clic en su ícono.
        Si falla → ícono rojo inmediato + abre el asistente de configuración.
        """
        svc = self.state.service
        self._snack(f"Validando sesión de {platform}…")
        try:
            if platform == "YouTube Music":
                ok = await svc.init_youtube()
                hint = "OK" if ok else (svc.youtube_auth_error or "Error — revisa browser.json")
            elif platform == "Spotify":
                ok = await svc.init_spotify()
                hint = "OK" if ok else "Error de credenciales Spotify"
            elif platform == "Apple Music":
                ok = await svc.init_apple()
                hint = "OK" if ok else "Faltan APPLE_AUTH_BEARER / APPLE_MUSIC_USER_TOKEN"
            else:
                return
        except Exception as exc:
            ok = False
        ...
        self.state.auth_session_ok[platform] = ok
        self.state.auth_session_hint[platform] = hint
        self.state.notify()

        if not ok:
            am = getattr(self, "auth_manager", None)
            if am:
                am.open_wizard(platform=platform)   # §2: navigate directly to failing tab
            else:
                self._snack(f"[{platform}] Sesión inválida: {hint}", error=True)
        else:
            self._snack(f"[{platform}] ✓ Sesión válida")
    ...
    def _close_postmortem_dialog(self) -> None:
        ...
        # FIX4: cerrar via overlay (compatible con Flet ≥ 0.21)
        dlg = getattr(self, "_postmortem_dlg", None)
        if dlg is not None:
            try:
                dlg.open = False
                self.page.update()
    ...
    def _build_sidebar(self) -> None:
        ...
        logo = ft.Column([
            ft.Row([
                ...
                ft.Column([
                    ft.Text("Playlist Manager", size=16, weight=ft.FontWeight.W_700,
                            color=TEXT_PRIMARY, font_family="IBM Plex Sans",
                            opacity=1.0),
                    ft.Text("v4.4", size=9, color=TEXT_DIM,
                            font_family="IBM Plex Sans",
        ...
        # ── Action buttons ─────────────────────────────────────────────
        self._load_btn     = _primary_btn("Cargar Playlist", ft.Icons.DOWNLOAD,    self._on_load)
        self._transfer_btn = _ghost_btn("Transferir",        ft.Icons.SWAP_HORIZ,  self._on_transfer)
        self._sync_btn     = _ghost_btn("Sincronizar",       ft.Icons.SYNC,        lambda _: self._snack("Próximamente"))
        self._split_btn    = _ghost_btn("Dividir",           ft.Icons.CALL_SPLIT,  lambda _: self._snack("Próximamente"))
        self._delete_btn   = _ghost_btn("Eliminar",          ft.Icons.DELETE_OUTLINE, lambda _: self._snack("Próximamente"))

        actions = ft.Column([
            self._load_btn,
            ft.Row([self._transfer_btn, self._sync_btn],   spacing=6),
            ft.Row([self._split_btn,    self._delete_btn], spacing=6),
        ], spacing=6)
        ...
        self._progress_label = ft.Text(
            "", size=10, color=TEXT_MUTED, font_family="IBM Plex Sans", opacity=1.0,
        )
        self._progress_row = ft.Container(
            content=ft.Column([
                self._progress_bar,
                ft.Row([self._progress_label],
                       alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
            ], spacing=4),
            bgcolor=BG_HOVER,
            border=ft.Border.all(0.8, BORDER_LIGHT),   # §3: turns SUCCESS on DONE
            border_radius=8,
            padding=ft.Padding.symmetric(horizontal=10, vertical=8),
            visible=False,
        )

        # ── Triple-counter panel ────────────────────────────────────────
        def _counter_row(label: str, color: str) -> ft.Row:
            val = ft.Text("—", size=11, color=color,
                          font_family="IBM Plex Sans", weight=ft.FontWeight.W_600,
                          opacity=1.0)
            lbl = ft.Text(label, size=9, color=TEXT_DIM, font_family="IBM Plex Sans",
                          opacity=1.0)
            return ft.Row([lbl, ft.Container(expand=True), val], spacing=0), val

        (row_det, self._cnt_detected),    = [_counter_row("Detectadas",  TEXT_MUTED)]
        (row_cand, self._cnt_candidates), = [_counter_row("Candidatas",  TEXT_MUTED)]
        (row_proc, self._cnt_processed),  = [_counter_row("Procesadas",  ACCENT)]
        (row_conf, self._cnt_confirmed),  = [_counter_row("Confirmadas", SUCCESS)]
        (row_rej,  self._cnt_rejected),   = [_counter_row("Rechazadas API", ERROR_COL)]

        self._counter_panel = ft.Container(
            content=ft.Column(
                controls=[row_det, row_cand, row_proc, row_conf, row_rej],
                spacing=3,
            ),
            bgcolor=BG_HOVER,
    ...
    def _build_content(self) -> None:
        ...
        self._auth_yt = ft.IconButton(
            ...
            on_click=lambda _: asyncio.create_task(
                self._on_platform_auth_click("YouTube Music")
            ),
            **_ib_style,
        )
        self._auth_sp = ft.IconButton(
            ...
            on_click=lambda _: asyncio.create_task(
                self._on_platform_auth_click("Spotify")
            ),
            **_ib_style,
        )
        self._auth_am = ft.IconButton(
            ...
            on_click=lambda _: asyncio.create_task(
                self._on_platform_auth_click("Apple Music")
            ),
            **_ib_style,
        )
        ...
        self._empty_state = ft.Container(
            ...
            content=ft.Column(
                controls=[
                    ...
                    ft.Text(
                        "Introduce el ID en el panel izquierdo y pulsa «Cargar playlist».",
                        size=12,
                        color=TEXT_DIM,
                        font_family="IBM Plex Sans",
                        opacity=1.0,
    ...
    def _on_state_changed(self) -> None:
        ...
        for plat, ic in _pam:
            ok = s.auth_session_ok.get(plat)  # None = checking, True = OK, False = fail
            if ok is None:
                ic.icon_color = TEXT_DIM      # neutral: still validating
            elif ok:
                ic.icon_color = SUCCESS
            else:
                ic.icon_color = ft.Colors.RED_ACCENT  # §1: RED_ACCENT on 401/403
            base = f"{plat}: clic para revalidar ahora"
            hint = s.auth_session_hint.get(plat) or ""
            status_word = "Validando…" if ok is None else ("OK" if ok else "fallo")
            ic.tooltip = f"{base} · {hint}" if hint else f"{base} · {status_word}"

        dest_ok = s.auth_session_ok.get(s.destination)  # None treated as "unknown"
        dest_ok_bool = dest_ok is True   # None (checking) or False → not OK
        self._dest_session_warn.visible = (dest_ok is False)
        self._dest_session_warn.value = (
            "" if dest_ok is not False else f"Sesión expirada en {s.destination}"
        )
        ...
        is_transfer_active = is_transferring or is_transfer_done
        # §3: panel always stays visible once activated — never hide post-DONE
        self._progress_row.visible  = is_transfer_active
        self._counter_panel.visible = is_transfer_active

        if s.transfer_total:
            # Progress bar based on count_confirmed (post-API reality) when done,
            # or on search progress during the running phase
            if is_transfer_done and s.count_detected:
            ...
            # §3: green border + green text + persistent message on DONE
            if is_transfer_done:
                self._progress_bar.color    = SUCCESS
                self._progress_label.color  = SUCCESS
                self._progress_row.border   = ft.Border.all(0.8, SUCCESS)
                no_tx = fallidas + rechazadas
                self._progress_label.value = (
                    f"✓ Transferencia Finalizada · {s.count_confirmed} confirmadas"
                    + (f" / {no_tx} no transferidas" if no_tx else "")
                )
            else:
                self._progress_bar.color    = ACCENT
                self._progress_label.color  = TEXT_MUTED
                self._progress_row.border   = ft.Border.all(0.8, BORDER_LIGHT)
                self._progress_label.value = (
                    f"{porcentaje}%  ·  "
        ...
        if is_transfer_active:
            self._cnt_detected.value   = str(s.count_detected)   if s.count_detected   else "—"
            self._cnt_candidates.value = str(s.count_candidates) if s.count_candidates else "—"
            self._cnt_processed.value  = str(s.count_processed)  if s.count_processed  else "—"
            self._cnt_confirmed.value  = str(s.count_confirmed)  if s.count_confirmed  else "—"
            self._cnt_rejected.value   = str(len(s.api_rejected_tracks)) or "—"

        # ── Log ────────────────────────────────────────────────────────
        self._log_box.visible = bool(s.log_lines)
        self._log_list.controls.clear()
        for line in s.log_lines[-60:]:
            col = SUCCESS if "[SUCCESS]" in line else (ERROR_COL if "[ERROR]" in line else TEXT_MUTED)
            self._log_list.controls.append(
                ft.Text(f"› {line}", size=9, color=col, font_family="IBM Plex Sans",
                        opacity=1.0)
            )
        ...
        self._transfer_btn.disabled = (
            net_blocked or is_transferring or not is_ready or not dest_ok_bool
        )
        self._transfer_btn.tooltip = (
            f"Sesión expirada en {s.destination}"
            if not dest_ok_bool
            else "Transferir selección al destino"
        )

        # ── Post-transfer dialog ───────────────────────────────────────
        # §4: fires on DONE *and* on ERROR (so partial failures are always reported)
        if s.transfer_state in (TransferState.DONE, TransferState.ERROR):
            self._show_failed_dialog()
        elif s.transfer_state == TransferState.RUNNING:
            self._failed_dialog_shown = False
    ...
    def _show_failed_dialog(self) -> None:
        s      = self.state
        failed   = getattr(s, "failed_tracks", [])
        rejected = getattr(s, "api_rejected_tracks", [])
        pending  = getattr(s, "pending_review_tracks", [])
        err_tx   = getattr(s, "transfer_error_tracks", [])
        failed_nf = [t for t in failed if t not in err_tx]
        if not failed_nf and not rejected and not pending and not err_tx:
            return
        if self._failed_dialog_shown:
            return
        self._failed_dialog_shown = True

        def _status_tag(t: Track) -> str:
            if t in pending or t.transfer_status == "revision_necesaria":
                return "PENDIENTE"
            if t in err_tx or t.transfer_status == "error":
                return "ERROR"
            if t in failed_nf:
                return "NO ENCONTR."
            if t in rejected:
        ...
        def _track_row(t: Track, icon: str, color: str) -> ft.Container:
            sub = (t.failure_reason or "").strip()
            return ft.Container(
                content=ft.Column([
                    ft.Row([
                        ft.Icon(icon, color=color, size=13),
                        ft.Text(
                            f"[{_status_tag(t)}] {t.name[:40]} — {t.artist[:26]}",
                    ...
                    ft.Text(
                        ...
                        opacity=0.9,
                    ) if sub else ft.Container(height=0),
                ], spacing=2, tight=True),
                padding=ft.Padding.symmetric(horizontal=4, vertical=4),
                border=ft.Border.only(bottom=ft.BorderSide(0.5, BORDER_LIGHT)),
            )
        ...
        if err_tx:
            all_items.append(
                ft.Container(
                    content=ft.Text(
                        f"ERRORES DE EJECUCIÓN ({len(err_tx)})",
                        size=8, color=TEXT_DIM, font_family="IBM Plex Sans",
                        weight=ft.FontWeight.W_700,
            ...
            all_items += [
                _track_row(t, ft.Icons.BOLT, ERROR_COL) for t in err_tx
            ]
        ...
        if pending:
            all_items.append(
                ft.Container(
                    content=ft.Text(
                        f"PENDIENTES DE REVISIÓN ({len(pending)})",
                        size=8, color=TEXT_DIM, font_family="IBM Plex Sans",
                        weight=ft.FontWeight.W_700,
                        style=ft.TextStyle(letter_spacing=1.2),
                        opacity=1.0,
            ...
            all_items += [
                _track_row(t, ft.Icons.FIND_IN_PAGE, WARNING) for t in pending
            ]
        ...
        if failed_nf:
            all_items.append(
                ft.Container(
                    content=ft.Text(
                        f"NO ENCONTRADAS ({len(failed_nf)})",
                        size=8, color=TEXT_DIM, font_family="IBM Plex Sans",
                        weight=ft.FontWeight.W_700,
                        style=ft.TextStyle(letter_spacing=1.2),
                        opacity=1.0,
            ...
            all_items += [
                _track_row(t, ft.Icons.SEARCH_OFF, ERROR_COL) for t in failed_nf
            ]
        ...
        if rejected:
            all_items.append(
                ft.Container(
                    content=ft.Text(
                        f"NO INSERTADAS POR LA PLATAFORMA ({len(rejected)})",
                        size=8, color=TEXT_DIM, font_family="IBM Plex Sans",
                        weight=ft.FontWeight.W_700,
                        style=ft.TextStyle(letter_spacing=1.2),
                        opacity=1.0,
            ...
            all_items += [
                _track_row(t, ft.Icons.BLOCK, WARNING) for t in rejected
            ]
        ...
        def _export_errors(_) -> None:
            log_path = "transfer_postmortem.txt"
            import datetime as _dt
            lines = [
                "# Playlist Manager v4.3 — Informe post-mortem\n",
                f"# Fecha: {_dt.datetime.now().isoformat()}\n",
                f"# Destino: {s.destination}\n",
                f"# Detectadas: {s.count_detected}  Procesadas: {s.count_processed}  "
                f"Confirmadas: {s.count_confirmed}\n\n",
            ]
            if err_tx:
                lines.append(f"## Errores de ejecución ({len(err_tx)})\n")
                for t in err_tx:
                    lines.append(
                        f"[ERROR] {t.name} | {t.artist} | {t.failure_reason or '—'}\n"
                    )
                lines.append("\n")
            if pending:
                lines.append(f"## Pendientes de revisión ({len(pending)})\n")
                for t in pending:
                    lines.append(
                        f"[PENDIENTE] {t.name} | {t.artist} | {t.failure_reason or 'fuzzy bajo'}\n"
                    )
                lines.append("\n")
            if failed_nf:
                lines.append(f"## No encontradas ({len(failed_nf)})\n")
                for t in failed_nf:
                    lines.append(
                        f"[NO ENCONTR.] {t.name} | {t.artist} | {t.failure_reason or '—'}\n"
                    )
                lines.append("\n")
            if rejected:
                lines.append(f"## No insertadas por la plataforma ({len(rejected)})\n")
                for t in rejected:
                    lines.append(f"{t.name} | {t.artist} | {t.album}\n")
            try:
                with open(log_path, "w", encoding="utf-8") as f:
                    f.writelines(lines)
                self._snack(f"✓ Log exportado → {log_path}")
            except Exception as exc:
                self._snack(f"Error al exportar: {exc}", error=True)

        _seen = set()
        total_errors = 0
        for t in failed_nf + rejected + pending + err_tx:
        ...
        dlg = ft.AlertDialog(
            modal=True,
            title=ft.Row([
                ft.Icon(ft.Icons.ERROR_OUTLINE, color=ERROR_COL, size=18),
                ft.Text(
                    f"Errores de transferencia · {total_errors} problema(s)  "
                    f"[{s.count_confirmed}/{s.count_detected} confirmadas]",
            ...
            content=ft.Container(
                content=failed_lv,
                width=520,
                height=min(360, total_errors * 34 + 80),
                bgcolor=BG_SURFACE,
                border_radius=8,
                padding=ft.Padding.all(4),
            ),
            actions=[
                ft.TextButton(
                    content=ft.Text("Exportar log (.txt)", opacity=1.0),
                    icon=ft.Icons.DOWNLOAD_OUTLINED,
                    on_click=_export_errors,
                    style=ft.ButtonStyle(color={ft.ControlState.DEFAULT: ACCENT}),
                ),
                ft.TextButton(
                    content=ft.Text("Cerrar", opacity=1.0),
        ...
        # FIX4: abrir via overlay (evita page.show_dialog deprecado)
        self._postmortem_dlg = dlg
...
async def main(page: ft.Page) -> None:
    try:
        ...
        _asyncio.get_event_loop().set_exception_handler(_exception_handler)
        ...
        # FIX2 — Validación proactiva de sesiones al arrancar la app
        async def check_all_sessions() -> None:
            """
            Llama a init_* de cada plataforma en secuencia y actualiza
            auth_session_ok / auth_session_hint para colorear íconos
            desde el primer fotograma, sin esperar ningún clic del usuario.
            """
            for platform, init_fn in (
                ("Spotify",       service.init_spotify),
                ("YouTube Music", service.init_youtube),
                ("Apple Music",   service.init_apple),
            ):
                try:
                    ok = await init_fn()
                except Exception:
                    ok = False
                state.auth_session_ok[platform] = ok
                if not ok and platform == "YouTube Music":
                    hint = service.youtube_auth_error or "Error — revisa browser.json"
                elif ok:
                    hint = "OK"
                else:
                    hint = "Error de credenciales"
                state.auth_session_hint[platform] = hint
                state.notify()   # actualiza íconos plataforma a plataforma

        asyncio.create_task(check_all_sessions())
        ...
        # ── Graceful shutdown (v4.3) ─────────────────────────────────────
        shutdown_event = asyncio.Event()
        _shutdown_once = [False]

        async def hard_cleanup() -> None:
            """
            Apagado seguro: detiene tareas de UI/lazy, cancela workers,
            cierra requests.Session (YTM / Spotify / Apple). Idempotente.
            """
            if _shutdown_once[0]:
                shutdown_event.set()
                return
            ...
            try:
                ...
                if ui._search_task and not ui._search_task.done():
                    ui._search_task.cancel()
                    ui._search_task = None
            except Exception:
                pass
            try:
                state.cancel_lazy_scan()
            except Exception:
                pass
            # FIX5 — cancelar transferencia activa si la hay
            try:
                if state.transfer_state == TransferState.RUNNING:
                    state.transfer_state = TransferState.ERROR
                    state.notify()
            except Exception:
                pass
            auth_poll = getattr(ui, "_auth_poll_task", None)
            if auth_poll and not auth_poll.done():
                auth_poll.cancel()
            auth_manager_task = getattr(auth_manager, "_reload_task", None)
            if auth_manager_task and not auth_manager_task.done():
                auth_manager_task.cancel()
            ...
            for task in asyncio.all_tasks():
                ...
                try:
                    co = task.get_coro()
                    qual = getattr(co, "__qualname__", "") if co else ""
                except Exception:
                    qual = ""
                if qual == "main":
                    continue
            ...
            gc.collect()
            await asyncio.to_thread(service._cleanup_sessions)
            shutdown_event.set()

        def _on_close(_) -> None:
            asyncio.create_task(hard_cleanup())

        page.on_close = _on_close

        await shutdown_event.wait()
    except asyncio.CancelledError:
        print("[INFO] Tareas canceladas. Cerrando motor de forma segura...")
    finally:
        try:
            await page.window.destroy()
        except Exception:
            pass
        # FIX5 — forzar muerte total del proceso (evita hilos zombie de requests/ytm)
        import os as _os
        _os._exit(0)
