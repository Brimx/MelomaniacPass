...
class Auth:
    ...
    def login_youtube(self):
        ...
        Variables requeridas en .env:
            YTM_COOKIE          → valor completo de la cabecera Cookie
            YTM_USER_AGENT      → User-Agent del navegador (opcional pero recomendado)
            YTM_AUTH_USER       → valor de X-Goog-AuthUser, normalmente "0" (opcional)
            YTM_SAPISID         → valor de X-Goog-VisitorId / SAPISID (opcional)
 
        Cómo obtener los valores (una sola vez):
            1. Abre music.youtube.com en Chrome/Firefox con tu cuenta iniciada.
            2. DevTools → Network → recarga → clic en cualquier petición a
               music.youtube.com → pestaña Headers → Request Headers.
            3. Copia el valor completo de 'Cookie' (es la línea más larga).
            4. Copia también 'User-Agent' y 'X-Goog-AuthUser' si aparecen.
            5. Pégalos en .env como YTM_COOKIE="...", YTM_USER_AGENT="..." etc.
 
        ytmusicapi acepta un dict de headers directamente (además de un archivo),
        lo que permite centralizar todas las credenciales en un solo .env.
        """
        ...
        sapisid    = os.getenv("YTM_SAPISID", "").strip()
        ...
        # Formato idéntico al que genera 'ytmusicapi browser' en browser.json,
        # pero construido en memoria desde las variables de entorno.
        headers = {
            "Cookie": cookie,
        ...
        if sapisid:
            # Algunas versiones de ytmusicapi y ciertas regiones requieren
            # este header para la validación SAPISIDHASH
            headers["X-Goog-VisitorId"] = sapisid
 
        # ── 3. Cargar sesión y validar con petición de prueba ──────────────
        try:
            client = YTMusic(headers_auth=headers)
