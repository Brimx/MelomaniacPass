...
try:
    from spotipy.cache_handler import CacheHandler as _SpotipyCacheHandler
    _CacheHandlerBase = _SpotipyCacheHandler
except ImportError:
...
class CacheHandler(_CacheHandlerBase):
    """
...
class CacheFileHandler(CacheHandler):
    ...
    def save_token_to_cache(self, token_info):
        try:
            ...
            logger.warning("Couldn't set permissions to cache file at: %s", self.cache_path)
        except OSError:
...
class DjangoSessionCacheHandler(CacheHandler):
    ...
    def save_token_to_cache(self, token_info):
        try:
            ...
            # Django session backends can raise heterogeneous errors with no common base class
            logger.warning("Error saving token to cache: %s", e)
...
class FlaskSessionCacheHandler(CacheHandler):
    ...
    def save_token_to_cache(self, token_info):
        try:
            ...
            # Flask session backends can raise heterogeneous errors with no common base class
            logger.warning("Error saving token to cache: %s", e)
...
class RedisCacheHandler(CacheHandler):
    ...
    def get_cached_token(self):
        # Importación diferida: evita ImportError al cargar el módulo cuando
        # redis-py no está instalado (sólo se necesita si se usa RedisCacheHandler).
        try:
            from redis import RedisError  # pylint: disable=import-outside-toplevel
        except ImportError as exc:
            raise RuntimeError(
                "redis-py is required to use RedisCacheHandler. "
        ...
        try:
            ...
            logger.warning("Error getting token from cache: %s", e)
...
class MemcacheCacheHandler(CacheHandler):
    ...
    def get_cached_token(self):
        try:
            from pymemcache import MemcacheError  # pylint: disable=import-outside-toplevel
        except ImportError as exc:
            raise RuntimeError(
                "pymemcache is required to use MemcacheCacheHandler. "
        ...
        try:
            ...
            logger.warning("Error getting token to cache: %s", e)
        return None
